...
command_actions! {
    ...
    Backspace ("backspace") => backspace |view, _window, cx| {
        if view.inline_rename_active() { view.backspace_inline_rename(cx); } else { view.dispatch(EditorCommand::Backspace, cx); }
    },
    Delete ("delete") => delete |view, _window, cx| {
        if view.inline_rename_active() { view.delete_inline_rename(cx); } else { view.dispatch(EditorCommand::Delete, cx); }
    },
    Left ("left") => left |view, _window, cx| {
        if view.inline_rename_active() { view.move_inline_rename_left(false, cx); } else { view.dispatch(EditorCommand::MoveLeft { extend: false }, cx); }
    },
    Right ("right") => right |view, _window, cx| {
        if view.inline_rename_active() { view.move_inline_rename_right(false, cx); } else { view.dispatch(EditorCommand::MoveRight { extend: false }, cx); }
    },
    Up ("up") => up |view, window, cx| {
        if !view.inline_rename_active() { view.move_vertical(false, false, window, cx); }
    },
    Down ("down") => down |view, window, cx| {
        if !view.inline_rename_active() { view.move_vertical(true, false, window, cx); }
    },
    SelectLeft ("shift-left") => select_left |view, _window, cx| {
        if view.inline_rename_active() { view.select_inline_rename_left(cx); } else { view.dispatch(EditorCommand::MoveLeft { extend: true }, cx); }
    },
    ...
    SelectUp ("shift-up") => select_up |view, window, cx| { if !view.inline_rename_active() { view.move_vertical(false, true, window, cx); } },
    SelectDown ("shift-down") => select_down |view, window, cx| { if !view.inline_rename_active() { view.move_vertical(true, true, window, cx); } },
    ...
    Home ("home") => home |view, _window, cx| {
        if view.inline_rename_active() { view.move_inline_rename_home(cx); } else { view.dispatch(EditorCommand::MoveToLineStart { extend: false }, cx); }
    },
    ...
    DocumentStart ("secondary-up") => document_start |view, _window, cx| { if !view.inline_rename_active() { view.dispatch(EditorCommand::MoveToStart { extend: false }, cx); } },
    DocumentEnd ("secondary-down") => document_end |view, _window, cx| { if !view.inline_rename_active() { view.dispatch(EditorCommand::MoveToEnd { extend: false }, cx); } },
    SelectDocumentStart ("secondary-shift-up") => select_document_start |view, _window, cx| { if !view.inline_rename_active() { view.dispatch(EditorCommand::MoveToStart { extend: true }, cx); } },
    SelectDocumentEnd ("secondary-shift-down") => select_document_end |view, _window, cx| { if !view.inline_rename_active() { view.dispatch(EditorCommand::MoveToEnd { extend: true }, cx); } },
    Undo ("secondary-z") => undo |view, _window, cx| { if !view.inline_rename_active() { view.dispatch(EditorCommand::Undo, cx); } },
    Redo ("secondary-shift-z") => redo |view, _window, cx| { if !view.inline_rename_active() { view.dispatch(EditorCommand::Redo, cx); } },
