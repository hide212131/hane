...
view_path = Path("crates/ui/src/view.rs")
...
replace_once(
    ...
    """use gpui::{
    App, Context, CursorStyle, FocusHandle, Focusable, InteractiveElement, IntoElement, MouseButton,
    MouseDownEvent, MouseMoveEvent, MouseUpEvent, ParentElement, PathPromptOptions, Render,
    ScrollHandle, ScrollWheelEvent, StatefulInteractiveElement, Styled, Subscription, Window, div,
    point, prelude::FluentBuilder, px, rgb,
};""",
    "gpui imports",
)
...
/// The width layout wraps against:""",
    """fn clamp_scroll_y(scroll_y: f32, content_height: f32, viewport_height: f32) -> f32 {
    let max_scroll = (content_height - viewport_height).max(0.0);
    scroll_y.clamp(0.0, max_scroll)
}
...
replace_once(
    ...
    """            selected_folder: None,
            ...
            pending_new_folders: HashSet::new(),""",
    "EditorView panel initialization",
)
...
marker = "impl Render for EditorView {"
if text.count(marker) != 1:
    raise SystemExit(f"render impl marker count: {text.count(marker)}")
text = text.replace(marker, helper_impl + marker, 1)
...
replace_once(
    ...
    "dynamic sidebar width in editor layout",
)
...
replace_once(
    ...
    "editor scrollbar child",
)

sidebar_start = text.index("impl EditorView {\n    /// The folder/file tree for the sidebar")
sidebar_end = text.index("\n/// The sidebar label for the work folder root", sidebar_start)
sidebar_impl = r'''impl EditorView {
...
'''
text = text[:sidebar_start] + sidebar_impl + text[sidebar_end:]
view_path.write_text(text)

icons_path = Path("crates/ui/src/icons.rs")
icons = icons_path.read_text()


def replace_icons(old: str, new: str, label: str) -> None:
    global icons
    count = icons.count(old)
    if count != 1:
        raise SystemExit(f"{label}: expected exactly one match, found {count}")
...
icons = icons.replace(
    "/// Serves the four work-folder sidebar icons from memory.",
    "/// Serves the work-folder sidebar icons from memory.",
    1,
)
icons_path.write_text(icons)

Path("assets/icons/work-folder/chevron-right.svg").write_text(
    '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="none">
  <path d="M6 4L10 8L6 12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
'''
)
Path("assets/icons/work-folder/chevron-down.svg").write_text(
    '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="none">
