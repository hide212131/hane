...
class RunBoundaryImeStepTests(unittest.TestCase):
    def _base_run_helper(self, fixture_path, *, type_romaji_ok=True,
                         type_romaji_effect=None, undo_save_ok=True,
                         click_text_ok=True, japanese_source_available=True):
        calls = {'type_romaji': 0, 'select_source': []}
        ...
        def fake_run_helper(swift_helper, args, timeout):
            ...
            if command == 'list-sources':
                ...
                if japanese_source_available:
                    sources += f'\n{interaction.JAPANESE_SOURCE}'
                return True, sources, ''
            if command == 'click-text':
                if not click_text_ok:
                    return False, '', 'OCR が対象文字列を認識できない'
                return True, '', ''
            if command == 'type-romaji-at-caret-commit-save':
                calls['type_romaji'] += 1
                if not type_romaji_ok:
                    return False, '', 'System Events を利用できない'
                if type_romaji_effect is not None:
                    type_romaji_effect(fixture_path)
                else:
                    expected = interaction.insert_at_match(
    ...
    def _run(self, fixture_path, **kwargs):
        fake_run_helper, calls = self._base_run_helper(fixture_path, **kwargs)
    ...
    def test_pass_path_converts_undoes_and_restores_original_source(self):
        with tempfile.TemporaryDirectory() as directory:
            fixture_path = Path(directory) / 'fixture.md'
            fixture_path.write_text(interaction.INLINE_FIXTURE_ORIGINAL, encoding='utf-8')
            steps, calls = self._run(fixture_path)
        ...
        undo_check = next(s for s in steps if s['name'] == 'boundary_ime_input_undo_check')
        self.assertEqual(undo_check['result'], 'pass')
        restore = next(s for s in steps if s['name'] == 'restore_boundary_ime_input_source')
        self.assertEqual(restore['result'], 'pass')
        self.assertEqual(calls['select_source'], ['com.apple.keylayout.ABC'])
        self.assertEqual(fixture_path.read_text(encoding='utf-8'), interaction.INLINE_FIXTURE_ORIGINAL)

    def test_no_japanese_source_is_blocked(self):
        with tempfile.TemporaryDirectory() as directory:
            fixture_path = Path(directory) / 'fixture.md'
            fixture_path.write_text(interaction.INLINE_FIXTURE_ORIGINAL, encoding='utf-8')
            steps, calls = self._run(fixture_path, japanese_source_available=False)

        self.assertEqual(calls['type_romaji'], 0)
        overall = next(s for s in steps if s['name'] == 'boundary_ime_input')
        self.assertEqual(overall['result'], 'blocked')
        self.assertIn('日本語入力ソース', overall['reason'])

    def test_helper_failure_is_blocked_not_failed(self):
        with tempfile.TemporaryDirectory() as directory:
            fixture_path = Path(directory) / 'fixture.md'
            fixture_path.write_text(interaction.INLINE_FIXTURE_ORIGINAL, encoding='utf-8')
            steps, calls = self._run(fixture_path, type_romaji_ok=False)
    ...
    def test_unconverted_mismatch_is_a_fail_not_a_silent_pass(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            fixture_path = Path(directory) / 'fixture.md'
            fixture_path.write_text(interaction.INLINE_FIXTURE_ORIGINAL, encoding='utf-8')
            steps, calls = self._run(fixture_path, type_romaji_effect=unconverted)
