...
pub(crate) struct MacPlatformState {
    ...
    keyboard_input_source_signature: Option<(String, bool)>,
...
fn current_keyboard_input_source_signature() -> Option<(String, bool)> {
    unsafe {
        let source = TISCopyCurrentKeyboardInputSource();
        if source.is_null() {
            return None;
        }
        let id: id = TISGetInputSourceProperty(source, kTISPropertyInputSourceID as *const c_void);
        let ascii_capable: id = TISGetInputSourceProperty(
            source,
            kTISPropertyInputSourceIsASCIICapable as *const c_void,
        );
        let signature = if id.is_null() {
            None
        } else {
            let id_utf8: *const c_char = msg_send![id, UTF8String];
            if id_utf8.is_null() {
                None
            } else {
                let ascii = if ascii_capable.is_null() {
                    true
                } else {
                    msg_send![ascii_capable, boolValue]
                };
                Some((
                    CStr::from_ptr(id_utf8).to_string_lossy().into_owned(),
                    ascii,
                ))
            }
        };
...
// AppKit does not reactivate the current first responder's text context when
// the user switches input source while a Hane window is already focused.
// Rebind that context once per actual source change so Japanese preedit starts.
...
fn reactivate_text_input_context_for_window(key_window: id) {
    ...
    unsafe {
        ...
        let accepts_context: BOOL = msg_send![responder, respondsToSelector: sel!(inputContext)];
        if accepts_context != YES {
            return;
        }
        let context: id = msg_send![responder, inputContext];
        if context.is_null() {
            return;
        }
        let already_reactivating = INPUT_CONTEXT_REACTIVATION_IN_PROGRESS.with(|flag| {
            if flag.get() {
                true
            } else {
                flag.set(true);
        ...
        if already_reactivating {
            return;
        }
        struct ResetReactivation;
        impl Drop for ResetReactivation {
            fn drop(&mut self) {
                INPUT_CONTEXT_REACTIVATION_IN_PROGRESS.with(|flag| flag.set(false));
            }
        }
        let _reset = ResetReactivation;
...
#[cfg(test)]
mod hane_input_source_tests {
    use super::*;
    use std::sync::atomic::{AtomicPtr, AtomicUsize, Ordering};

    static TEST_WINDOW: AtomicPtr<Object> = AtomicPtr::new(ptr::null_mut());
    static ACTIVATIONS: AtomicUsize = AtomicUsize::new(0);
    static DEACTIVATIONS: AtomicUsize = AtomicUsize::new(0);

    extern "C" fn first_responder(this: &Object, _: Sel) -> id {
    ...
    extern "C" fn activate(_: &Object, _: Sel) {
        ACTIVATIONS.fetch_add(1, Ordering::SeqCst);
    }

    extern "C" fn deactivate(_: &Object, _: Sel) {
        DEACTIVATIONS.fetch_add(1, Ordering::SeqCst);
        // AppKit can synchronously emit another input-source notification.
        reactivate_text_input_context_for_window(TEST_WINDOW.load(Ordering::SeqCst));
    }

    fn test_classes() -> (*const Class, *const Class, *const Class) {
        static CLASSES: OnceLock<(usize, usize, usize)> = OnceLock::new();
        let (window, responder, context) = *CLASSES.get_or_init(|| unsafe {
            let mut context = ClassDecl::new("HanePreImeTestContext", class!(NSObject)).unwrap();
            context.add_method(sel!(activate), activate as extern "C" fn(&Object, Sel));
            context.add_method(sel!(deactivate), deactivate as extern "C" fn(&Object, Sel));
            let context = context.register() as *const Class as usize;

            let mut responder =
                ClassDecl::new("HanePreImeTestResponder", class!(NSObject)).unwrap();
            ...
            let mut window = ClassDecl::new("HanePreImeTestWindow", class!(NSObject)).unwrap();
            window.add_ivar::<id>("firstResponder");
            window.add_method(
                sel!(firstResponder),
                first_responder as extern "C" fn(&Object, Sel) -> id,
            );
            let window = window.register() as *const Class as usize;
            (window, responder, context)
        });
        (
            window as *const Class,
            responder as *const Class,
            context as *const Class,
        )
    }

    #[test]
    fn hane_input_source_reactivation_is_guarded_and_null_safe() {
        ACTIVATIONS.store(0, Ordering::SeqCst);
        DEACTIVATIONS.store(0, Ordering::SeqCst);
        reactivate_text_input_context_for_window(nil);

        let (window_class, responder_class, context_class) = test_classes();
        unsafe {
            let window: id = msg_send![window_class, new];
            TEST_WINDOW.store(window, Ordering::SeqCst);
            reactivate_text_input_context_for_window(window);

            let non_text_responder: id = msg_send![class!(NSObject), new];
            (*window).set_ivar("firstResponder", non_text_responder);
            reactivate_text_input_context_for_window(window);

            let responder: id = msg_send![responder_class, new];
            (*window).set_ivar("firstResponder", responder);
            reactivate_text_input_context_for_window(window);

            let context: id = msg_send![context_class, new];
            (*responder).set_ivar("inputContext", context);
            reactivate_text_input_context_for_window(window);
            assert_eq!(DEACTIVATIONS.load(Ordering::SeqCst), 1);
            assert_eq!(ACTIVATIONS.load(Ordering::SeqCst), 1);

            TEST_WINDOW.store(ptr::null_mut(), Ordering::SeqCst);
            let _: () = msg_send![context, release];
            let _: () = msg_send![responder, release];
            let _: () = msg_send![non_text_responder, release];
