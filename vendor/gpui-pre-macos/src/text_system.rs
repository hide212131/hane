...
use core_text::{
    ...
    font_descriptor::{
        CTFontDescriptor, SymbolicTraitAccessors, kCTFontSlantTrait, kCTFontSymbolicTrait,
...
// tan(12 degrees), matching the synthetic italic used for Japanese fallback.
const SYNTHETIC_OBLIQUE_SHEAR: f32 = 0.212_556_56;
...
impl MacTextSystemState {
    // A synthetic face needs a distinct FontId so its raster cache cannot
    // alias the upright glyph while retaining the selected face's advances.
    ...
    fn raster_bounds(&self, params: &RenderGlyphParams) -> Result<Bounds<DevicePixels>> {
        ...
        // Font-kit uses downward-positive Y; Core Graphics draws upward.
        ...
        // Preserve the upstream antialiasing margin for both native and
        // synthetic glyphs; the transformed outline fits in these bounds.
    ...
    fn layout_line(&mut self, text: &str, font_size: Pixels, font_runs: &[FontRun]) -> LineLayout {
        ...
        // CoreText carries this run attribute across fallback font selection.
...
mod tests {
    ...
    use gpui::{
        DevicePixels, FontRun, GlyphId, IsZero, PlatformTextSystem, RenderGlyphParams, font, point,
        px,
    };

    #[test]
    fn hane_oblique_fallback_changes_raster_without_moving_glyphs() {
        ...
        for bold in [false, true] {
            ...
            let normal_id = fonts.font_id(&regular).unwrap();
            ...
            let normal = layout(normal_id);
            let italic = layout(italic_id);
            ...
            for ((normal_face, normal_glyph), (italic_face, italic_glyph)) in
                normal_glyphs.iter().zip(italic_glyphs.iter())
            {
                assert_ne!(normal_face, italic_face, "italic ink needs its own FontId");
                assert_eq!(normal_glyph.id, italic_glyph.id);
                assert_eq!(normal_glyph.position, italic_glyph.position);
                assert_eq!(normal_glyph.index, italic_glyph.index);
                for scale_factor in [1.0, 2.0] {
                    for subpixel_variant in [point(0, 0), point(1, 1)] {
                        let normal_params = RenderGlyphParams {
                            font_id: *normal_face,
                            glyph_id: normal_glyph.id,
                            font_size: px(32.0),
                            subpixel_variant,
                            scale_factor,
                            is_emoji: false,
                        ...
                        let (italic_size, italic_pixels) = fonts
                            ...
                            .unwrap();
                        assert!(italic_pixels.iter().any(|pixel| *pixel != 0));
                        assert_ne!(normal_pixels, italic_pixels, "compare actual glyph images");

                        let mut padded = italic_bounds;
                        padded.origin.x -= DevicePixels(8);
                        padded.origin.y -= DevicePixels(8);
                        padded.size.width += DevicePixels(16);
                        padded.size.height += DevicePixels(16);
                        let (reference_size, reference) =
                            fonts.rasterize_glyph(&italic_params, padded).unwrap();
                        for y in 0..reference_size.height.0 as usize {
                            for x in 0..reference_size.width.0 as usize {
                                let pixel = reference[y * reference_size.width.0 as usize + x];
                                if x >= 8
                                    && y >= 8
                                    ...
                                    && y < italic_size.height.0 as usize + 8
                                {
                                    ...
                                    assert_eq!(pixel, 0, "oblique ink exceeds raster bounds");
                                }
                            }
                        }
                    }
            ...
            assert_eq!(normal.runs[0].font_id, layout(normal_id).runs[0].font_id);
            assert_eq!(italic.runs[0].font_id, layout(italic_id).runs[0].font_id);
        }
    }

    #[test]
    fn hane_oblique_preserves_native_italic_emoji_and_blank_glyphs() {
        let fonts = MacTextSystem::new();
        ...
        for run in &layout.runs {
            for glyph in &run.glyphs {
                if glyph.index < 3 || glyph.is_emoji {
        ...
        let japanese_id = fonts.font_id(&font("Hiragino Sans").italic()).unwrap();
        for ch in [' ', '\u{3000}'] {
            let glyph_id = fonts.glyph_for_char(japanese_id, ch).unwrap();
            ...
            assert!(fonts.glyph_raster_bounds(&params).unwrap().is_zero());
        }
    }
