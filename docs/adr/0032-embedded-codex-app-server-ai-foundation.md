...
秘密の保存成功後に非秘密設定をatomic writeする。失敗時は以前の有効設定を維持し、中途半端な切替を避ける。設定エラーを黙ってデフォルト接続へ戻さない。
...
API key環境変数の子ツールへの継承を防ぐ。現行仕様ではshell_environment_policy.filters等があるが、設定の存在だけでなく、実プロセスで秘密が漏れないことを検証する。MCPや別経路の子プロセスも将来追加時に対象とする。[S3]
