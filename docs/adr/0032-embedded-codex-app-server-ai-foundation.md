...
資格情報の更新では、現在の`credential_ref`を上書きしない。Haneが新しいCredentialRefの識別子を秘密の保存前に割り当て、更新操作全体を再起動後も追跡できる永続credential operation journalを使う。journalには`update`または`delete`の操作種別、新旧CredentialRef、操作状態、設定世代など復旧に必要な非秘密情報だけを保存し、秘密値は保存しない。
...
6. Hane起動時にoperation journalを操作種別に応じて処理する。`update`では、現行設定が新CredentialRefを参照していれば新側を保持し、旧CredentialRefの削除を再試行する。現行設定が旧CredentialRefを参照していれば旧側を保持し、未参照の新側を削除する。`delete`では、現行設定が旧CredentialRefをまだ参照していれば、まず`credential_ref`を外す設定変更をatomic replaceで再試行し、その成功後に旧CredentialRefの削除を再試行する。現行設定が旧CredentialRefを参照していなければ、旧CredentialRefが未参照であることを再確認して削除だけを再試行する。journalと現行設定が整合しない場合は秘密を自動採用・削除せず、診断対象として残す。
7. `update`では設定切替と不要な資格情報の削除、`delete`では`credential_ref`を外す設定変更と旧資格情報の削除がすべて成功した後だけjournalから操作記録を除く。どれかが失敗した場合はjournalを保持し、次回起動時に未完了の処理を再試行する。OS資格情報ストアが列挙を提供する場合は、Hane専用namespace内の未参照項目との照合も補助的なgarbage collectionとして行うが、列挙機能だけには依存しない。
