...
### Phase 6a: pre-GUI routing `fix` からの自動 Claude 修正

Phase 5a までで、Codex findings を Copilot pre-GUI routing に渡し、対象 head SHA に `hane/copilot-routing` として `fix` / `continue-validation` / `blocked` を確定させる経路が自動化されている。Phase 6a はその続きとして、`fix` が確定した時点で人間の `/implement` や `/claude-fix` コメントなしに Claude Code の修正 worker を自動起動する。

実装は次の3ファイルに分かれる。

- `.github/workflows/copilot-routing.yml`: routing 結果を `hane/copilot-routing` に保存した直後（新規判定・再利用のどちらの経路でも）、決定が `fix` なら `repository_dispatch` イベント `hane-claude-fix`（`client_payload` に `pr_number` と `target_sha`）を `GITHUB_TOKEN` で発行する。`continue-validation` / `blocked` では発行しない。
- `.github/workflows/claude-fix.yml`: `hane-claude-fix` を受けて起動する新しい worker workflow。
- `.github/workflows/codex-review.yml`: Claude の修正 push 後に新しい head SHA を最初から再レビューできるよう、`workflow_dispatch`（`pr_number` / `target_sha` 入力）を追加する。

#### トリガーの信頼境界

`claude-fix.yml` は dispatch payload の `pr_number` / `target_sha` を鵜呑みにしない。起動直後に次をすべて再確認し、1つでも満たさなければ fail closed で終了する。

- 対象 Pull Request が open かつ non-draft であること。
- `head.repo.full_name` が同一リポジトリであること（fork PR は対象外）。
- PR 作者が owner / write / maintain / admin のいずれか、または信頼済み bot（`github-actions[bot]` / `claude[bot]`）であること。
- 対象 branch が `main` ではないこと。
- 再取得した現在の head SHA が dispatch payload の `target_sha` と一致すること（不一致なら stale として何もせず終了する）。
- 対象 SHA 上の `hane/copilot-routing` 最新 status が `state=failure` かつ `description` が厳密に `Copilot routing: fix for <short-sha>` であること。これ以外の値（`continue-validation` / `blocked` / 不正な文字列など）では起動しない。

#### 既存 Pull Request branch への実装境界

Claude Code Action v1 は `repository_dispatch` を安全にサポートするが、`pull_request` に紐づかないイベントから起動した場合に既存 PR branch へ確実に push する挙動を、この worker の設計時点では検証できなかった。誤って第二の実装 Pull Request を作ることを避けるため、`claude-fix.yml` は次の構成を採用する。

1. `actions/checkout@v5` で対象 PR の head branch（`head.ref`）を明示的に checkout する。
2. `claude` CLI（`@anthropic-ai/claude-code`）を `-p`（非対話）モードで直接起動し、`CLAUDE_CODE_OAUTH_TOKEN` を使う。Claude には `git commit` / `git push` を含む git 操作を一切行わないよう指示し、`--allowedTools` は `cargo fmt` / `cargo check` / `cargo test` / `cargo clippy` の Bash 呼び出しに限定する。
3. Claude の作業結果は commit 前の working tree の差分として残す。
4. push 直前に現在の PR head を再取得し、dispatch 時点の `target_sha` と一致することを確認する（2つ目の exact-head guard）。一致しなければ変更を commit も push もせず、`main` への push が発生しないことを保証したまま stale として終了する。
5. 一致していれば workflow 自身が `git commit` し、`head.ref` に対して `git push` する（`main` への push は明示的に拒否する）。

この構成を Phase 6a の effect boundary として採用する。将来 Claude Code Action 側で「既存 PR branch への確実な push」が公式に保証された場合は、このステップを Action 呼び出しに置き換えてよい。

#### GITHUB_TOKEN による push は下流 workflow を再起動しない

`claude-fix.yml` の push は既定の `GITHUB_TOKEN` で行う。GitHub Actions は `GITHUB_TOKEN` が起こしたイベント（push、コメント、status 更新など）からは `workflow_dispatch` / `repository_dispatch` 以外の新しい workflow run を起動しない。これは `implement.yml` の `dispatch_ci` が `pull_request` トリガーに頼らず `gh workflow run ci.yml` で明示的に CI を再起動しているのと同じ制約である。

そのため `claude-fix.yml` は push 成功後、新しい head SHA に対して次を明示的に再起動する。

- `ci.yml` を `workflow_dispatch`（`target_sha` / `generation_id`）で再起動し、`dispatch_ci` と同じ手順で `hane/trusted-ci-generation` と各プラットフォームの CI status を `pending` にしてから dispatch する。
- `codex-review.yml` を `workflow_dispatch`（`pr_number` / `target_sha`）で再起動する。`workflow_dispatch` 経由の実行は常に `review_mode=manual` として扱われ、`CODEX_GITHUB_TOKEN` を使った明示的な `@codex review` コメントの経路を通る。

Codex が新しい head に対して submitted review を投稿すると、その `pull_request_review: submitted` イベントは Codex 自身のアカウントが起こす通常のイベントであり `GITHUB_TOKEN` 制約の対象外なので、`copilot-routing.yml` の既存 `pull_request_review` トリガーがそのまま連鎖する。したがって `copilot-routing.yml` 自体を明示的に再起動する必要はない。

#### 渡す証跡と信頼境界

Claude に渡すのは次に限定する。

- Pull Request 番号と対象 head SHA
- 対象 head SHA の commit に紐づく Codex submitted review の本文と inline finding（`path` / `line` / `body`）
- 対象 head SHA に投稿された Copilot pre-GUI routing の `fix` 理由コメント
- 変更ファイルのメタデータ（`filename` / `status` / `additions` / `deletions`）

これらはすべて `evidence.json` としてまとめ、prompt 内で明示的に **untrusted data** と注記する。埋め込み指示（secret の要求、セキュリティ制御の変更要求、この contract の上書き要求など）には従わないことも prompt に明記する。

#### `hane/claude-fix` の状態

`hane/claude-fix` は `hane/codex-review` / `hane/copilot-routing` と同じ commit status ベースの状態管理を使い、対象 head SHA に次のいずれかを保存する。

| description | state | 意味 |
| --- | --- | --- |
| `Claude fix pending for <short-sha>` | pending | worker が起動し、証跡収集前 |
| `Claude fix running for <short-sha>` | pending | Claude CLI 実行中 |
| `Claude fix completed for <short-sha>` | success | 新しい commit を push 済み。新しい head SHA は同じ description に添えたコメントおよび `target_url`（`.../commit/<new-sha>`）から追跡できる |
| `Claude fix blocked: max iterations for <short-sha>` | error | 3回反復に到達し、これ以上自動実行しない |
| `Claude fix blocked: missing evidence for <short-sha>` | error | Codex findings が exact-SHA で確認できない |
| `Claude fix failed for <short-sha>` | error | Claude CLI 実行が失敗した |
| `Claude fix produced no changes for <short-sha>` | error | Claude が working tree を変更しなかった |
| `Claude fix stale for <short-sha>` | error | push 直前に head が動いていた |
| `Claude fix controller failed for <short-sha>` | error | workflow 自体が異常終了した |

`success` の対象 SHA が mergeable であることは意味しない。古い SHA 上のこの status は履歴上の証跡としてのみ扱う。

#### 重複排除・lease・再試行

`(Pull Request 番号, target SHA, hane/claude-fix)` を dedup の単位とする。`claude-fix.yml` は Claude 実行前に対象 SHA の `hane/claude-fix` 最新 status を確認し、次のように振る舞う。

- `Claude fix completed` / 各種 `blocked` 系の terminal error が既にあれば、何もせず終了する（重複実行しない）。
- `Claude fix pending` / `Claude fix running` が既にあり、その status の `created_at` から 55 分未満しか経過していなければ、in-flight とみなして何もせず終了する。
- 55 分以上経過していれば、abandoned lease とみなして再試行する（同じ transition のやり直しであり、新しい反復としては数えない）。

`copilot-routing.yml` は `fix` を新規判定した場合だけでなく、既存の terminal `fix` を再利用した場合（例: `/copilot-routing` の再実行）にも `hane-claude-fix` を dispatch する。これは重複配送を許容し、受信側の `hane/claude-fix` lease チェックで冪等性を保証する設計であり、`docs` 冒頭で述べた「dispatcher の再試行と receiver 側の重複排除を分離する」方針に沿う。

#### 3回反復の上限

Claude が push した commit は Pull Request の diff に残り続けるため、`pulls/{pr}/commits` で列挙できる。`claude-fix.yml` は対象 SHA を除くこれらの commit のうち `hane/claude-fix` が `Claude fix completed for <short-sha>` になっているものの数を数え、3以上であれば新しい修正を起動せず `Claude fix blocked: max iterations for <short-sha>` を `error` として保存し、Pull Request に人間向けコメントを残す。

#### 認証情報の分離

- `CLAUDE_CODE_OAUTH_TOKEN` は `claude` CLI の起動にのみ使う。
- `CODEX_GITHUB_TOKEN` は `codex-review.yml` 内の `@codex review` コメント投稿にのみ使う（`claude-fix.yml` からは直接使わない）。
- `COPILOT_GITHUB_TOKEN` は `copilot-routing.yml` の Copilot 推論にのみ使う。
- それ以外の GitHub API 呼び出し（PR 取得、status 投稿、`workflow_dispatch` / `repository_dispatch` の発行、git push）はすべて `GITHUB_TOKEN` を最小権限で使う。

この Issue の範囲では Local GUI validation・Copilot final judge・deterministic merge gate・自動 merge は起動しない。`fix` は必ず Claude 修正 worker だけを起動し、`continue-validation` / `blocked` では Claude を起動しない。
...
### Phase 6a: Claude 自動修正

- `copilot-routing.yml` が対象 head SHA に terminal `fix` を保存した直後（新規判定・再利用のどちらでも）、`repository_dispatch` イベント `hane-claude-fix` を `GITHUB_TOKEN` で発行する。
- `claude-fix.yml` は dispatch payload を鵜呑みにせず、PR の状態・trust・対象 branch・現在 head との一致・`hane/copilot-routing` が exact-SHA で `fix` であることを再確認してから起動する。
- Claude は既存 PR head branch を checkout した working tree に対して `claude` CLI を直接実行し、git 操作は行わない。workflow が push 直前に exact-head を再確認してから commit / push する。
- push 後、`ci.yml` と `codex-review.yml` を `workflow_dispatch` で明示的に再起動し、新しい head SHA から検証をやり直す。`GITHUB_TOKEN` が起こしたイベントは他の workflow を自動的に再起動しないため、この明示的な再起動が必要になる。
- 対象 SHA には `hane/claude-fix` を保存し、`(Pull Request 番号, target SHA, hane/claude-fix)` 単位で重複実行を防ぐ。lease は 55 分で期限切れとみなし、それ以降は再試行を許可する。
- 3回反復の上限に達したら `blocked` として自動実行を止める。
- Local GUI validation・Copilot final judge・deterministic merge gate・自動 merge はこの Phase では起動しない。
