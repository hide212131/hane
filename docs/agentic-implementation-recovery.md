...
Claude の実装ステップが終了したら、停止ステップが `success` / `failure` 等の固定された
step outcome を表示して監視を終える。同じジョブ内で動くため、この時点では GitHub の
実装ジョブ自体はまだ `completed` ではない。表示する結果は Claude の実装ステップの結果であり、
後続ステップ・CI・GUI 検証の成功ではない。
...
job token（`actions: read` を含む）を使うが、監視スクリプトの API 操作はコメントとジョブの読み取りだけで、
