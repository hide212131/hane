...
The snapshot includes the judge procedure version (`copilot-final/2`), so correcting the invocation or decision policy does not reuse a terminal result from an older procedure. The CLI receives a nonempty allowlist containing no actual tool, plus shell/write/URL denial and disabled built-in MCPs. An empty CLI allowlist would mean unspecified, and `--deny-tool '*'` is unsupported. Invocation diagnostics are bounded and redact supplied credential values.
